...
public class Sword : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if ((garouHealth <= 0 || erinaHealth <= 0) && erinavsgarou == 0)
        {
            ...
            {
                if (animator.GetBool("batonAttack"))
                {
                    animator.SetBool("batonAttack", false);
                }

                if (animator.GetBool("batonAttackLeft"))
                {
        ...
        ahmettnyCurrentHealth = ahmettnyHealth;
        ahmettnyHealthbar.SetHealth(ahmettnyCurrentHealth);

        authoryCurrentHealth = authoryHealth;
        authoryHealthbar.SetHealth(authoryCurrentHealth);

        garouCurrentHealth = garouHealth;
        garouHealthbar.SetHealth(garouCurrentHealth);

        erinaCurrentHealth = erinaHealth;
        erinaHealthbar.SetHealth(erinaCurrentHealth);

        nevoksaCurrentHealth = nevoksaHealth;
        nevoksaHealthbar.SetHealth(nevoksaCurrentHealth);

        darkGarouCurrentHealth = darkGarouHealth;
        darkGarouHealthbar.SetHealth(darkGarouCurrentHealth);
    }

    private void OnCollisionEnter2D(Collision2D collision)
    {
        if (collision.gameObject.tag != "shield" && (gameObject.tag == "sword" || gameObject.name == "garou" || gameObject.name == "darkGarou"))
        {
            ...
            if (collision.gameObject.name == "authory" && gameObject.name == "stick" && collision.gameObject.name != "earth")
            {
                authoryHealth--;
